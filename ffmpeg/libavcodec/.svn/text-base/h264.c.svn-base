...
static int decode_slice_header(H264Context *h, H264Context *h0){
    ...
    if (!s->context_initialized) {
		...
		av_log(h->s.avctx, AV_LOG_WARNING, "Codec id: %d format: %d ptr: %p .\n", s->avctx->codec->id, s->avctx->pix_fmt, s->avctx->hwaccel);
    ...
    if(h0->current_slice == 0){
        ...
        if(!FIELD_PICTURE || s0->first_field) {
            if (ff_h264_frame_start(h) < 0) {
                s0->first_field = 0;
                av_log(h->s.avctx, AV_LOG_ERROR, "ff_h264_FRAME_START\n");
				return -1;
            }
        } else {
    ...
    if(h->slice_type_nos!=AV_PICTURE_TYPE_I && ff_h264_decode_ref_pic_list_reordering(h) < 0) {
		...
		av_log(h->s.avctx, AV_LOG_ERROR, "ff_h264_ref_pic_list_reordering\n");
        return -1;
    }
    ...
    if(h->nal_ref_idc && ff_h264_decode_ref_pic_marking(h0, &s->gb) < 0 &&
       (s->avctx->err_recognition & AV_EF_EXPLODE))
	{
		av_log(h->s.avctx, AV_LOG_ERROR, "ref_pic_making\n");
        return AVERROR_INVALIDDATA;
	}
    if(FRAME_MBAFF){
