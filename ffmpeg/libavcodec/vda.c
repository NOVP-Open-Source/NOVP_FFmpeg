...
#undef printf
#undef fprintf
//fprintf(stderr, "VDA Decoding ON\n");
//	av_log(NULL, AV_LOG_WARNING, "VDA Decoding ON\n");
    if (!image_buffer)
        return;
...
int ff_vda_create_decoder(struct vda_context *vda_ctx,
                          ...
                          int extradata_size)
{
	av_log(NULL, AV_LOG_WARNING, "VDA Creating decoder\n");
	...
	av_log(NULL, AV_LOG_WARNING, "Enum id: %d, fmt: %d\n", CODEC_ID_H264, PIX_FMT_VDA_VLD);
	...
	av_log(NULL, AV_LOG_WARNING, "VDA Status: %d\n", status);
                          ...
                          int64_t frame_pts)
{
	...
	//av_log(NULL, AV_LOG_WARNING, "ff_vda_decoder_decode: %d \n", status);
    if (kVDADecoderNoErr != status)
        return status;
