...
#define FF_ALLOC_EVENT   (EVENTTYPE_USEREVENT)
#define FF_REFRESH_EVENT (EVENTTYPE_USEREVENT + 1)
#define FF_QUIT_EVENT    (EVENTTYPE_USEREVENT + 2)
...
static void write_to_shmem(VideoState* is, mp_image_t *img)
{
    is->shmem=1000;
    if(is->shmem && img && !is->shmempriv) {
fprintf(stderr,"open shmem: %d\n",is->shmem);
        is->svideosize=sizeof(shmem_video_t)+(img->w*img->h*img->bpp/8);
        is->svideo = calloc(1,is->svideosize);
        set_shmem_size(SHMEM_TYPE_RAWDATA, is->svideosize);
        is->shmempriv=shmem_open(is->shmem, 1);
        is->svideo->id=SHMEM_VIDEO_PACKET;
        is->svideo->format=img->imgfmt;
        is->svideo->width=img->w;
        is->svideo->height=img->h;
        is->svideo->aspect=0;
        is->svideo->size=(img->w*img->h*img->bpp/8);
        is->svideo->bytes=0;
        is->svideo->pts=0.0;
    }
    if(is->shmempriv && img) {
        char* dst=is->svideo->data;
        memcpy(dst,img->planes[0],img->stride[0]*img->h);
        dst+=img->stride[0]*img->h;
        if(img->planes[1] && img->stride[1] && img->chroma_height) {
            memcpy(dst,img->planes[1],img->stride[1]*img->chroma_height);
            dst+=img->stride[1]*img->chroma_height;
        }
        if(img->planes[2] && img->stride[2] && img->chroma_height) {
            memcpy(dst,img->planes[2],img->stride[2]*img->chroma_height);
            dst+=img->stride[1]*img->chroma_height;
        }
        shmem_write(is->shmempriv, (unsigned char *)is->svideo, is->svideosize);
    }
}
...
static void stream_close(VideoState *is)
{
    ...
    if (is->img) {
        free_mp_image(is->img);
        is->img = NULL;
    }
    if(is->shmempriv)
        shmem_close(is->shmempriv);
...
#if 0
    is->shmem=1000;
    if(is->shmem && !is->shmempriv) {
        is->svideosize=sizeof(shmem_video_t)+(is->img->w*is->img->h*is->img->bpp/8);
        is->svideo = calloc(1,is->svideosize);
        set_shmem_size(SHMEM_TYPE_RAWDATA, is->svideosize);
        is->shmempriv=shmem_open(is->shmem, 1);
        is->svideo->id=SHMEM_VIDEO_PACKET;
        is->svideo->format=is->img->imgfmt;
        is->svideo->width=is->img->w;
        is->svideo->height=is->img->h;
        is->svideo->aspect=0;
        is->svideo->size=is->svideosize;
        is->svideo->bytes=0;
        is->svideo->pts=0.0;
    }
#endif
...
static void* refresh_thread(void *opaque)
{
    ...
    while(!is->abort_request){
        ...
        if(!is->refresh){
            is->refresh=1;
            push_event(is->eventqueue, &event);
        }
...
static void alloc_picture(void *opaque)
{
    ...
    if(!is->img) {
        is->img = alloc_mpi(vp->width, vp->height, IMGFMT_YV12);
    }
    vp->bmp = alloc_mpi(vp->width, vp->height, IMGFMT_YV12);
...
//    SDL_AudioSpec wanted_spec, spec;
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO) {
        wanted_spec.format = AF_FORMAT_S16_NE;
#warning FIXME!!
...
#endif
        if (spec.format != AF_FORMAT_S16_NE) {
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for(;;) {
        event = wait_event(cur_stream->eventqueue);
        if(!event)
            break;
        switch(event->type) {
        case FF_QUIT_EVENT:
...
//    SDL_EventState(SDL_USEREVENT, SDL_IGNORE);
    ...
    getchar();
    /* never returns */
    uninit_eventqueue(is->eventqueue);
    return 0;
}
